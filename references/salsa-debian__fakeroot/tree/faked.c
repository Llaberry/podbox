...
void process_stat(struct fake_msg *buf){
  ...
  if (i == data_end()) {
    ...
    struct fakestat *cached = data_node_get(i);
    const mode_t real_type = buf->st.mode & S_IFMT;
    const mode_t cached_type = cached->mode & S_IFMT;

    /* If a file or directory gets unlinked without
     * fakeroot's knowledge and the inode gets recycled for
     * an entity of the "opposite" type we invalidate it,
     * assuming that there is no legitimate case for fakeroot
     * to think that a non-directory is a directory, or
     * vice-versa.
     */
    if ((real_type == S_IFDIR) != (cached_type == S_IFDIR)) {
      if (debug) {
        fprintf(stderr, "FAKEROOT: inode's file type is stale, dropping from cache ");
        debug_stat(cached);
      }
      data_erase(i);
      if (!unknown_is_real) {
...
# ifdef FAKEROOT_FAKENET
      cr_ret = close_range(0, sd-1, 0);
      if (cr_ret == 0)
        cr_ret = close_range(sd+1, num_fds, 0);
# else /* !FAKEROOT_FAKENET */
      cr_ret = close_range(0, num_fds, 0);
# endif
      /* Fall back to the explicit loop on any failure (e.g. EPERM
         from a Docker/seccomp sandbox), not just ENOSYS. */
      if (cr_ret != 0) {
