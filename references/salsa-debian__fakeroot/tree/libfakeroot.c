...
#define SEND_GET_UNLINK(a,b) send_get_unlink(a,b)
#define SEND_GET_UNLINK64(a,b) send_get_unlink64(a,b)
...
#define SEND_GET_UNLINK(a,b) send_get_unlink(a)
#define SEND_GET_UNLINK64(a,b) send_get_unlink64(a)
...
extern void send_get_unlink64_time64(const struct stat64 *st
#ifdef STUPID_ALPHA_HACK
                                     , int ver
#endif
                                    );
...
#define SEND_GET_UNLINK64_TIME64(a,b) send_get_unlink64_time64(a,b)
#else
#define SEND_GET_STAT64_TIME64(a,b) send_get_stat64_time64(a)
#define SEND_GET_UNLINK64_TIME64(a,b) send_get_unlink64_time64(a)
#endif
...
#define INT_SEND_GET_UNLINK(a) SEND_GET_UNLINK64_TIME64(a,_STAT_VER)
...
#define INT_SEND_GET_UNLINK(a) SEND_GET_UNLINK64(a,_STAT_VER)
#else
...
#define INT_SEND_GET_UNLINK(a) SEND_GET_UNLINK(a,_STAT_VER)
#endif
#endif /* !TIME64_HACK */
...
int unlinkat(int dir_fd, const char *pathname, int flags){
  ...
  if((flags & AT_REMOVEDIR) ? (S_ISDIR(st.st_mode) && st.st_nlink==2)
                             : (st.st_nlink==1))
    INT_SEND_GET_UNLINK(&st);
  else
    INT_SEND_STAT(&st, unlink_func);
...
int remove(const char *pathname){
  ...
  if(r)
    return -1;
  if((S_ISDIR(st.st_mode) && st.st_nlink==2) || st.st_nlink==1)
    INT_SEND_GET_UNLINK(&st);
  else
    INT_SEND_STAT(&st,unlink_func);
...
struct nftw_ctx {
  ...
  struct nftw_ctx *previous;
};

static __thread struct nftw_ctx *current_nftw_ctx;

static int fakeroot_nftw_callback(const char *path, const struct stat *st,
                                  int typeflag, struct FTW *ftwbuf)
{
  struct nftw_ctx *ctx = current_nftw_ctx;
  struct stat fake_st;
  ...
  if (!st || typeflag == FTW_NS) {
    return ctx->callback(path, st, typeflag, ftwbuf);
  }

  fake_st = *st;
  SEND_GET_STAT(&fake_st, _STAT_VER);

  return ctx->callback(path, &fake_st, typeflag, ftwbuf);
}
...
{
  struct nftw_ctx ctx = {.callback = fn, .previous = current_nftw_ctx};
  int r;

  current_nftw_ctx = &ctx;
  r = next_nftw(path, fakeroot_nftw_callback, fd_limit, flags);
  current_nftw_ctx = ctx.previous;
